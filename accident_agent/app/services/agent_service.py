...
# class AccidentAgent:
#     def __init__(self):
#         # Initialize OpenRouter using configurations from central settings
#         self.llm = ChatOpenAI(
#             base_url="https://openrouter.ai/api/v1",
...
#     def generate_initial_analysis(self, keyframes):
#         """Triggered automatically right after the accident is detected."""
        
#         # 1. Fetch relevant legal context dynamically from local FAISS/MiniLM vector store
#         # You can expand the query string as needed depending on your general dataset focus
#         legal_context = self.rag.get_relevant_law(
#             query="right of way intersection roundabout traffic lights overtaking rules"
#         )
        
#         # 2. Format the production system prompt from app.core.prompts
#         system_prompt = SYSTEM_PROMPT_TEMPLATE.format(legal_context=legal_context)
        
#         # Append to conversational memory state
#         self.chat_history.append(SystemMessage(content=system_prompt))
        
#         # 3. Assemble visual keyframes for the vision-language model (VLM)
#         human_content = [{"type": "text", "text": "Analyze these sequential frames:"}]
#         for path in keyframes:
#             if os.path.exists(path):
...
#         # 4. Invoke OpenRouter model and store the analysis response in context memory
#         response = self.llm.invoke(self.chat_history)
#         self.chat_history.append(AIMessage(content=response.content))
...
#     def chat_with_user(self, user_message):
#         """Handles follow-up questions after the initial automated analysis."""
#         self.chat_history.append(HumanMessage(content=user_message))
#         response = self.llm.invoke(self.chat_history)
#         self.chat_history.append(AIMessage(content=response.content))
#         return response.content